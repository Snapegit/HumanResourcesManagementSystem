	...
	<div class="app-contain" :style='{"padding":"0 12%","margin":"10px auto 20px","alignItems":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">工作年限</div>
					<div  class="info_text" >{{detail.gongzuonianxian}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'投递')" @click="toudijianlionAcross('投递','toudijianli','','[1]','已投递')" type="warning">投递</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('zhaopinxinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('zhaopinxinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="岗位详情" name="first">
				<div v-html="detail.gangweixiangqing"></div>
			</el-tab-pane>
		</el-tabs>
	</div>
</template>
<script setup>
	...
	const toudijianlionAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
