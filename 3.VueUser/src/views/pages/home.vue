	...
	<div>
		<div class="home_box">
			...
			<div class="recomList_view">
				<div class="recomList_title">招聘信息推荐</div>
				<div class="recommend_list_one">
					<div class="recommend_item animation_box" v-for="(item,index) in zhaopinxinxiRecomList" :key="index" @click="detailClick('zhaopinxinxi',item.id)">
						<div class="recommend_img_box">
							<img class="recommend_img" v-if="isHttp(item.bangonghuanjing)" :src="item.bangonghuanjing.split(',')[0]" alt="">
							<img class="recommend_img" v-else :src="item.bangonghuanjing?$config.url + item.bangonghuanjing.split(',')[0]:''" alt="">
						</div>
						<div class="recommend_content">
							<div class="recommend_title">
								{{item.gangweimingcheng}}
							</div>
							<div class="recommend_title">
								薪资待遇：{{item.xinzidaiyu}}
							</div>
							<div class="recommend_title">
								专员名称：{{item.zhuanyuanmingcheng}}
							</div>
				...
				<div class="recommend_more_view" @click="moreClick('zhaopinxinxi')">
					<span class="recommend_more_text">全部</span>
				</div>
			</div>
			<!-- 培训信息首页展示 -->
			<div class="homeList_view">
				<div class="homeList_title">培训信息展示</div>
				<div class="home_list_five">
					<div v-for="(item,index) in peixunxinxiHomeList" :class="'home_list_five_item' + (Number(index) + 1)">
						<div class="img_box">
				...
				<div class="homeList_more_view" @click="moreClick('peixunxinxi')">
					<span class="homeList_more_text">全部</span>
				</div>
			</div>
			<!-- 薪酬标准首页展示 -->
			<div class="homeList_view">
				<div class="homeList_title">薪酬标准展示</div>
				<div class="home_list_five">
					<div v-for="(item,index) in xinchoubiaozhunHomeList" :class="'home_list_five_item' + (Number(index) + 1)">
						<div class="img_box">
				...
				<div class="homeList_more_view" @click="moreClick('xinchoubiaozhun')">
					<span class="homeList_more_text">全部</span>
				</div>
			</div>
			<!-- 公告信息 -->
...
<script setup>
	...
	//招聘信息推荐
	const zhaopinxinxiRecomList = ref([])
	const getzhaopinxinxiRecomList = () => {
		let autoSortUrl = 'zhaopinxinxi/autoSort'
		context?.$http({
			url: autoSortUrl,
			method: 'get',
			params: {
			...
			zhaopinxinxiRecomList.value = res.data.data.list
		})
	}
	//培训信息首页展示
	const peixunxinxiHomeList = ref([])
	const getpeixunxinxiHomeList = () => {
		context?.$http({
			url: 'peixunxinxi/list',
			method: 'get',
			params: {
				page: 1,
				limit: 6
			...
			peixunxinxiHomeList.value = res.data.data.list
		})
	}
	//薪酬标准首页展示
	const xinchoubiaozhunHomeList = ref([])
	const getxinchoubiaozhunHomeList = () => {
		context?.$http({
			url: 'xinchoubiaozhun/list',
			method: 'get',
			params: {
				page: 1,
				limit: 6
			...
			xinchoubiaozhunHomeList.value = res.data.data.list
		})
	}
	//公告信息弹窗
	...
	const init = () => {
		//招聘信息推荐
		getzhaopinxinxiRecomList()
		//培训信息首页展示
		getpeixunxinxiHomeList()
		//薪酬标准首页展示
		getxinchoubiaozhunHomeList()
		//公告信息
		getNewsList()
	}
	init()
