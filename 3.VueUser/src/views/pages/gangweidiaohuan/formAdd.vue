	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0","margin":"20px auto 60px","borderRadius":"0px","background":"#fff","width":"76%","position":"relative","height":"100%"}'>
		...
		<el-form ref="formRef" :model="form" class="add_form" label-width="120px" :rules="rules">
			<el-row>
				...
				<el-col :span="8">
					<el-form-item label="原职位" prop="zhiwei">
						<el-input class="list_inp" v-model="form.zhiwei" placeholder="原职位"
							 type="text" 							:readonly="!isAdd||disabledForm.zhiwei?true:false" />
					</el-form-item>
				</el-col>

				<el-col :span="8">
					<el-form-item label="调换职位" prop="diaohuanzhiwei">
						<el-input class="list_inp" v-model="form.diaohuanzhiwei" placeholder="调换职位"
							 type="text" 							:readonly="!isAdd||disabledForm.diaohuanzhiwei?true:false" />
					</el-form-item>
				</el-col>
				...
				<el-col :span="8">
					<el-form-item label="专员账号" prop="zhuanyuanzhanghao">
						<el-select
							class="list_sel"
							:disabled="!isAdd||disabledForm.zhuanyuanzhanghao?true:false"
							v-model="form.zhuanyuanzhanghao" 
							placeholder="请选择专员账号"
							style="width:100%;"
							@change="zhuanyuanzhanghaoChange">
							<el-option v-for="(item,index) in zhuanyuanzhanghaoLists" :label="item" :value="item">
							</el-option>
						</el-select>
					</el-form-item>
				</el-col>
				...
				<el-col :span="24">
					<el-form-item label="申请原因" prop="shenqingyuanyin">
						<el-input v-model="form.shenqingyuanyin" placeholder="申请原因" type="textarea"
						:readonly="!isAdd||disabledForm.shenqingyuanyin?true:false"
						/>
					</el-form-item>
				</el-col>
			</el-row>
...
<script setup>
	...
	//专员账号列表
	const zhuanyuanzhanghaoLists = ref([])
	//methods
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='diaohuanzhiwei'){
					form.value.diaohuanzhiwei = row[x];
					disabledForm.value.diaohuanzhiwei = true;
					continue;
				}
		...
		context?.$http({
			...
			if(json.hasOwnProperty('zhiwei') && context?.$toolUtil.storageGet("frontRole")!="管理员"){
				form.value.zhiwei = json.zhiwei
				disabledForm.value.zhiwei = true;
			}
		})
		context?.$http({
			url: `option/renshizhuanyuan/zhuanyuanzhanghao`,
			method: 'get'
		}).then(res=>{
			zhuanyuanzhanghaoLists.value = res.data.data
		})
		//由上级字段带出不可改
		disabledForm.value.zhuanyuanmingcheng = true;
	}
	...
	//专员账号回调
	const zhuanyuanzhanghaoChange=()=>{
		context?.$http({
			url: `follow/renshizhuanyuan/zhuanyuanzhanghao?columnValue=` + form.value.zhuanyuanzhanghao,
			method: 'get'
		}).then(res=>{
			//带出专员名称字段
			if(res.data.data.zhuanyuanmingcheng){
				form.value.zhuanyuanmingcheng = res.data.data.zhuanyuanmingcheng
			}
		})
	}
	//提交
