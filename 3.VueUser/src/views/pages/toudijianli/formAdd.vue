			...
			<el-row>
				...
				<el-col :span="12">
					<el-form-item label="个人简历" prop="gerenjianli">
						<uploads
							:disabled="!isAdd||disabledForm.gerenjianli?true:false"
							type="file"
							action="file/upload" 
							tip="请上传个人简历" 
							:limit="1" 
							style="width: 100%;text-align: left;"
							:fileUrls="form.gerenjianli?form.gerenjianli:''" 
							@change="gerenjianliUploadSuccess">
						</uploads>
					</el-form-item>
				</el-col>
				<el-col :span="8">
					<el-form-item label="投递时间" prop="toudishijian">
						<el-date-picker
							class="list_date"
							v-model="form.toudishijian"
							format="YYYY-MM-DD HH:mm:ss"
							value-format="YYYY-MM-DD HH:mm:ss"
							type="datetime"
							style="width:100%;"
							:readonly="!isAdd||disabledForm.toudishijian?true:false"
							placeholder="请选择投递时间" />
					</el-form-item>
				</el-col>
				<el-col :span="8">
					<el-form-item label="面试状态" prop="mianshizhuangtai">
						<el-select
							class="list_sel"
							:disabled="!isAdd||disabledForm.mianshizhuangtai?true:false"
							v-model="form.mianshizhuangtai" 
							placeholder="请选择面试状态"
							style="width:100%;"
							>
							<el-option v-for="(item,index) in mianshizhuangtaiLists" :label="item"
								:value="item"
								>
							</el-option>
						</el-select>
...
<script setup>
	...
	//个人简历上传回调
	const gerenjianliUploadSuccess=(e)=>{
		form.value.gerenjianli = e
	}
	//面试状态列表
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='gerenjianli'){
					form.value.gerenjianli = row[x];
					disabledForm.value.gerenjianli = true;
					continue;
				}
				if(x=='toudishijian'){
					form.value.toudishijian = row[x];
					disabledForm.value.toudishijian = true;
					continue;
				}
				if(x=='mianshizhuangtai'){
					form.value.mianshizhuangtai = row[x];
					disabledForm.value.mianshizhuangtai = true;
					continue;
				}
				if(x=='crossuserid'){
			...
			form.value.mianshizhuangtai='未面试'
		}
		context?.$http({
	...
	const save=()=>{
		...
		if(form.value.gerenjianli!=null) {
			form.value.gerenjianli = form.value.gerenjianli.replace(new RegExp(context?.$config.url,"g"),"");
		}
